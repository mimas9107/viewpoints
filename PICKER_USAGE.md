...
1. 點擊「匯出設定 (本地)」按鈕
2. 根據目前「選擇清單」產生並下載 `viewpoints.json` 檔案
3. 此方式適合在尚未儲存到伺服器前，先下載一份備份

**方式三：下載目前配置 (從伺服器)**

1. 點擊「下載目前配置 (從伺服器)」按鈕
2. 系統會從伺服器抓取您目前已儲存的個人化設定並下載
3. 此方式下載的是伺服器上的實際版本
...
- **清空** - 清除所有已選擇的監控點
- **儲存到伺服器** - 將目前選擇的配置寫入伺服器的個人設定中
- **匯出設定 (本地)** - 根據目前選擇產生 `viewpoints.json` 並下載
- **下載目前配置 (從伺服器)** - 下載伺服器上目前儲存的設定
- **預覽監控牆** - 顯示目前的選擇與版面設定資訊
