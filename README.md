...
基于 tw.live 的客制化監視器墙系统，可通过 JSON 組態檔轻松管理您关注的監控点。

## 功能特色

- 🎯 2x2 四宫格版面設定（可扩展到 3x2、3x3、4x3）
- 🔄 自动重新整理功能（可自訂重新整理间隔）
- 📱 回應式设计，支援手机和电脑
- 🖼️ 点击圖片可全屏查看
- ⚙️ 通过 JSON 檔案轻松設定監控点
...
啟動后，浏览器会自动打开 `http://localhost:8000`，或手动存取该位址。
...
|------|------|--------|
| `title` | 页面標題 | "我的監視器墙" |
| `autoRefresh` | 是否自动重新整理 | `true` 或 `false` |
| `refreshInterval` | 重新整理间隔（秒） | `60` |
...
### 手动重新整理
点击页面右上角的"手动重新整理"按鈕可立即更新所有監控畫面。

### 自动重新整理
- 点击"啟用自动重新整理"按鈕开启自动重新整理
- 重新整理间隔可在 `viewpoints.json` 中的 `refreshInterval` 設定（单位：秒）
- 点击"停止自动重新整理"可以关闭
...
A: 这是浏览器的安全性策略。**不要直接双击打开 HTML 檔案**，必須通过本機伺服器執行：
```bash
python3 start-server.py
```
...
解决方法：点击錯誤提示可重新載入，或到 tw.live 確認该監控点是否正常。
...
### Q: YouTube 直播会自动重新整理吗？

A: 不需要！YouTube 直播是即時视频流，会持续播放，无需重新整理。自动重新整理功能仅对靜態圖片監控器有效。
