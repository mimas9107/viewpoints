...
- `.env`: 環境變數設定檔。
- `.backups/`: 自動備份目錄。
- `scraper.py`: 主要爬蟲程式，使用預定義的分類列表進行抓取成"全域資料庫"用途。`v2.2.4`
- `scraper2.py`: 使用分類藍圖進行測試抓取成測試資料庫比對用途，須與scraper_blueprint.py搭配使用。`v2.2.4`
- `scraper_blueprint.py`: 分類發現工具，遍歷全站並收集所有監控點分類頁面。`v2.2.4`
...
- [x] `SCRAPER.md` - 更新爬蟲使用指南（新增 v2.2.4 技術重點整理）
